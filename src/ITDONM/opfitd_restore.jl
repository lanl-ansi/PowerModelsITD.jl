...
function constraint_boundary_voltage_magnitude_ONM_McCormick(pm::_PM.AbstractBFModel, pmd::_PMD.LPUBFDiagPowerModel, i::Int, f_idx::Tuple{Int,Int,Int}, f_connections::Vector{Int}, t_connections::Vector{Int}; nw::Int=nw_id_default)
    ...
    z_block = _ONM.var(pmd, nw, :z_block, _ONM.ref(pmd, nw, :bus_block_map, t_bus))
    # distribution bounds
    d1u = min(_PMD.ref(pmd, nw, :bus)[t_bus]["vmax"][1]^2, 1E6^2)
    d2u = min(_PMD.ref(pmd, nw, :bus)[t_bus]["vmax"][2]^2, 1E6^2)
    d3u = min(_PMD.ref(pmd, nw, :bus)[t_bus]["vmax"][3]^2, 1E6^2)
    d1l = max(_PMD.ref(pmd, nw, :bus)[t_bus]["vmin"][1]^2, 0^2)
    d2l = max(_PMD.ref(pmd, nw, :bus)[t_bus]["vmin"][2]^2, 0^2)
...
function constraint_thermal_limit_from_PM_lin(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    ...
    if haskey(branch, "rate_a")
        constraint_thermal_limit_from_PM_lin(pm, nw, f_idx, branch["rate_a"])
    end
end

function constraint_thermal_limit_from_PM_lin(pm::_PM.AbstractPowerModel, n::Int, f_idx, rating::Real)
    pf = _PM.var(pm, n, :p, f_idx)
    qf = _PM.var(pm, n, :q, f_idx)

    pf_sqr = JuMP.@variable(pm.model, base_name="$(n)_pf_sqr_$(f_idx)")
    qf_sqr = JuMP.@variable(pm.model, base_name="$(n)_qf_sqr_$(f_idx)")



        pf_lb, pf_ub = _IM.variable_domain(pf)
        PolyhedralRelaxations.construct_univariate_relaxation!(pm.model, x->x^2, pf, pf_sqr, [pf_lb, pf_ub], false)

        qf_lb, qf_ub = _IM.variable_domain(qf)
        PolyhedralRelaxations.construct_univariate_relaxation!(pm.model, x->x^2, qf, qf_sqr, [qf_lb, qf_ub], false)


    JuMP.@constraint(pm.model, sum(pf_sqr .+ qf_sqr) <= rating^2)
end

function constraint_thermal_limit_to_PM_lin(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    branch = _PM.ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    t_idx = (i, t_bus, f_bus)
    ...
    if haskey(branch, "rate_a")
        constraint_thermal_limit_to_PM_lin(pm, nw, t_idx, branch["rate_a"])
    end
end

function constraint_thermal_limit_to_PM_lin(pm::_PM.AbstractPowerModel, n::Int, t_idx, rating::Real)
    pt = _PM.var(pm, n, :p, t_idx)
    qt = _PM.var(pm, n, :q, t_idx)

    pt_sqr = JuMP.@variable(pm.model, base_name="$(n)_pt_sqr_$(t_idx)")
    qt_sqr = JuMP.@variable(pm.model, base_name="$(n)_qt_sqr_$(t_idx)")



        pt_lb, pt_ub = _IM.variable_domain(pt)
        PolyhedralRelaxations.construct_univariate_relaxation!(pm.model, x->x^2, pt, pt_sqr, [pt_lb, pt_ub], false)

        qt_lb, qt_ub = _IM.variable_domain(qt)
        PolyhedralRelaxations.construct_univariate_relaxation!(pm.model, x->x^2, qt, qt_sqr, [qt_lb, qt_ub], false)


    JuMP.@constraint(pm.model, sum(pt_sqr .+ qt_sqr) <= rating^2)
end
...
function build_mn_opfitd_restore(pmitd::AbstractBFPowerModelITD)   
    ...
    for (n, network) in nws(pmitd)
        ...
        for i in _PM.ids(pm_model, :branch, nw=n)
            ...
            constraint_thermal_limit_from_PM_lin(pm_model, i, nw=n)
            constraint_thermal_limit_to_PM_lin(pm_model, i, nw=n)
        end
    ...
    objective_min_shed_load_block_mod(pmitd)
end

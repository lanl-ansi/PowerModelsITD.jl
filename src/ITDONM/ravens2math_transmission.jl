...
function _get_bus_from_load(ld)
    bus = 0
    lb = loads_rv[ld]["ConductingEquipment.Terminals"][1]["Terminal.ConnectivityNode"]
    lb_key = split.(lb, "'")[2]
...
function _get_key_from_pointer(pointer)
...
function transform_data_model_ravens_transmission(nw)
    ...
    for xx in present_buses
        ...
        elseif "ConnectivityNode::'"*bus_key*"'" in gbs
  ...
  # prepare gens (quadratic costs only)
    ...
    branch_keys = collect(Iterators.flatten([y, z]))
    branch_ind = 0

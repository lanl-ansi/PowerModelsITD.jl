...
FOTRU boundary bus voltage magnitude constraints.
"""
function constraint_boundary_voltage_magnitude(pmd::_PMD.FOTRUPowerModel, ::Int, t_bus::Int, ::Vector{Int}, ::Vector{Int}; nw::Int=nw_id_default)
...
FOTRU boundary bus voltage angle constraints.
"""
function constraint_boundary_voltage_angle(pmd::_PMD.FOTRUPowerModel, ::Int, t_bus::Int, ::Vector{Int}, ::Vector{Int}; nw::Int=nw_id_default)
...
Generates the ACR-FOTRU boundary linking vars vector to be used by the StsDOpt Optimizer.
...
"""
function generate_boundary_linking_vars(pm::_PM.ACRPowerModel, pmd::_PMD.FOTRUPowerModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)
...
Generates the FOTRUPowerModel boundary linking vars vector to be used by the StsDOpt Optimizer.
The parameter `export_models` is a boolean that determines if the JuMP models' shared variable indices are exported to the pwd as .txt files.
"""
function generate_boundary_linking_vars_distribution(pmd::_PMD.FOTRUPowerModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)

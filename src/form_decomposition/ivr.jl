...
Generates the IVR-IVRU boundary linking vars vector to be used by the StsDOpt Optimizer.
...
Generates the IVR boundary linking vars vector to be used by the StsDOpt Optimizer.
The parameter `export_models` is a boolean that determines if the JuMP models' shared variable indices are exported to the pwd as .txt files.
"""
function generate_boundary_linking_vars_transmission(pm::_PM.AbstractIVRModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)
...
Generates the IVRU boundary linking vars vector to be used by the StsDOpt Optimizer.
The parameter `export_models` is a boolean that determines if the JuMP models' shared variable indices are exported to the pwd as .txt files.
"""
function generate_boundary_linking_vars_distribution(pmd::_PMD.IVRUPowerModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)

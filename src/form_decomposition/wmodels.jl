...
Generates the SOCBF-SOCNL boundary linking vars vector to be used by the StsDOpt Optimizer.
...
Generates the SOCBF boundary linking vars vector to be used by the StsDOpt Optimizer.
The parameter `export_models` is a boolean that determines if the JuMP models' shared variable indices are exported to the pwd as .txt files.
"""
function generate_boundary_linking_vars_transmission(pm::_PM.AbstractSOCBFModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)
...
Generates the SOCNLPUBF boundary linking vars vector to be used by the StsDOpt Optimizer.
The parameter `export_models` is a boolean that determines if the JuMP models' shared variable indices are exported to the pwd as .txt files.
"""
function generate_boundary_linking_vars_distribution(pmd::_PMD.SOCNLPUBFPowerModel, boundary_number::String; nw::Int=nw_id_default, export_models::Bool=false)

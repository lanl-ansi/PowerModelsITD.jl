...
#     end
    ...
    @testset "solve_opfitd_decomposition: Multi-System case500-caseLV (5 ds in ms) with 3 DGs NFA-NFAU" begin
        ...
        @info "--------------------------- Boundary flows --------------------------------"
        # 2. Boundary flows
        # @info "Boundary Flow (ITD) ckt 1: $(result_itd["solution"]["it"]["pmitd"]["boundary"])"
        @info "Boundary Flow (ITD) ckt 1: $(result_itd["solution"]["it"]["pmitd"]["boundary"]["(100001, 11, 253)"]["pbound_fr"])"
        @info "Boundary Flow (ITD) ckt 2: $(result_itd["solution"]["it"]["pmitd"]["boundary"]["(100002, 323, 254)"]["pbound_fr"])"
        @info "Boundary Flows (DECOMPOSITION) ckt 1: Pload = $(result_decomposition["it"]["pm"]["solution"]["boundary"]["(100002, 323, 127)"]["pbound_load"]) and Paux = $(result_decomposition["it"]["pmd"]["ckt_1"]["solution"]["boundary"]["(100002, 323, 127)"]["pbound_aux"])"
        @info "Boundary Flows (DECOMPOSITION) ckt 2: Pload = $(result_decomposition["it"]["pm"]["solution"]["boundary"]["(100001, 11, 127)"]["pbound_load"]) and Paux = $(result_decomposition["it"]["pmd"]["ckt_2"]["solution"]["boundary"]["(100001, 11, 127)"]["pbound_aux"])"
        @info "------------------ Generator Dispatches - Transmission --------------------"
        # # 3. Generator Dispatches - Transmission
        # @info "Gen Dispatches (ITD) - Transmission: $(result_itd["solution"]["it"]["pm"]["gen"])"
        # @info "Gen Dispatches (DECOMPOSITION) - Transmission: $(result_decomposition["it"]["pm"]["solution"]["gen"])"
